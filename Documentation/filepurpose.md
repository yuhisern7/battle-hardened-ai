...
- relay/.env.example — Example relay environment template with all configuration options documented.
- relay/documents/aitrainingcommands.md — Relay AI training command reference and usage examples.
- relay/documents/firewall.md — Relay server firewall configuration guide for relay server ports (60001 WebSocket, 60002 API).
- relay/init_relay_files.py — Relay JSON/directory initialization script (creates ai_training_materials/ structure).
- relay/documents/MODEL_DISTRIBUTION_PROOF.md — Documentation proving relay model distribution is working correctly.
- relay/documents/RELAY_SETUP.md — Complete relay server deployment guide.
...
- relay/setup.sh — Convenience script to install Docker/firewall rules and launch the relay stack.
