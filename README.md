...
| 21 | Email/SMS Alerts (Critical Events) | Configuration and runtime status for critical out‑of‑band alerts: which destinations are configured, which events (system failure, kill‑switch changes, integrity breaches) will trigger notifications, recent send history, and failure diagnostics—positioned as a narrow safety‑of‑operation channel rather than a full alerting platform. |
