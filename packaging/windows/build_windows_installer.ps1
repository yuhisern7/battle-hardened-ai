...
# Paths for existing EXE build (produced by build_windows_exe.ps1 in this folder)
$distDir   = Join-Path $scriptDir "dist"
$exePath   = Join-Path $distDir "BattleHardenedAI.exe"

if (-not (Test-Path $exePath)) {
    Write-Host "[build_windows_installer] BattleHardenedAI.exe not found, invoking build_windows_exe.ps1" -ForegroundColor Cyan
    $buildScript = Join-Path $scriptDir "build_windows_exe.ps1"
    ...
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $exePath)) {
        Write-Error "EXE build failed or BattleHardenedAI.exe was not created at $exePath."
        exit 1
    }
}
