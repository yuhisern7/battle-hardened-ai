...
- **Status**: ⚠️ PLANNED — Modules exist but enterprise_integration.json automatic streaming is not yet implemented. Adapters are export-only and do not control the first-layer firewall enforcement path (enforcement remains on the local OS firewall driven by threat_log.json and blocked_ips.json).
