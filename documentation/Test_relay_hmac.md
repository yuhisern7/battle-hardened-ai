...
```bash
python3 - <<'PY'
import hmac, hashlib, json, pathlib
key = pathlib.Path("relay/crypto_keys/shared_secret.key").read_bytes()
msg = json.dumps({"test": "relay-hmac", "version": 1}, separators=(",", ":")).encode()
